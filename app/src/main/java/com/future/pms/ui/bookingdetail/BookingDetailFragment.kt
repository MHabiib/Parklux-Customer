...
class BookingDetailFragment : Fragment(), BookingDetailContract {
        ...
        for (index in 0 until slotsLayout.length) {
            ...
            when {
                slotsLayout[index] == SLOT_NULL -> {
                    setupParkingView(
                        index, parkingLayout, slotsLayout[index], STATUS_ROAD,
                        R.drawable.ic_blank
                    )
                }
                slotsLayout[index] == SLOT_SCAN_ME || slotsLayout[index] == SLOT_TAKEN -> {
                    setupParkingView(
                        index, parkingLayout, slotsLayout[index], STATUS_BOOKED,
                        R.drawable.ic_car
                    )
                }
                slotsLayout[index] == SLOT_EMPTY -> {
                    setupParkingView(
                        index, parkingLayout, slotsLayout[index], STATUS_AVAILABLE,
                        R.drawable.ic_park
                    )
                }
                slotsLayout[index] == DISABLED_SLOT -> {
                    setupParkingView(
                        index, parkingLayout, slotsLayout[index], STATUS_RESERVED,
    ...
    private fun setupParkingView(
        count: Int, layout: LinearLayout?, code: Char, tags: Int,
        icon: Int
    ): TextView {
